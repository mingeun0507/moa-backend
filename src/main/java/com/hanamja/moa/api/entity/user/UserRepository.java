...
public interface UserRepository extends JpaRepository<User, Long> {
    ...
    List<User> findTop20ByRoleAndIsOnboardedAndIsActiveOrderByPointDescNameAsc(Role role, Boolean isOnboarded, Boolean isActive);

    @Query(value = "SELECT COUNT(u) FROM User u " +
            "WHERE u.point >= (SELECT distinct uu.point FROM User uu WHERE uu.id = :uid AND uu.role = :role AND u.isActive = true AND u.isOnboarded = true) AND u.role = :role ")
